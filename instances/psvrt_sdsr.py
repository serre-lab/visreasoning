...
class psvrt_sdsr(feeders.Feeder):
    ...
    def initialize_vars(self,
                        ...
                        position_sampler=None, item_sampler=None, organization = 'raw',
                        display=False):
        ...
        if self.organization != 'raw':
            self.actual_input_size = self.raw_input_size[0:2] + [self.num_items*self.raw_input_size[2]]
        else:
...
#            raise ValueError('Num items other than 2 not implemented yet.')
        ...
        while iimage < self.batch_size:
            ...
            elif self.problem_type == 'SDSR':
                if SD_label == SR_label:
                    label_batch[iimage] = 0
                else:
                    label_batch[iimage] = 1
            else:
        ...
        if (self.organization == 'obj') | (self.organization == 'full'):
            ...
            if self.display:
                for i in range(len(items_list)):
                    plt.subplot(self.num_items,i+1)
    ...
    def sample_positions(self, SR_label=None, SR_portion=1, SR_type = 'average_orientation'):
        ...
        if SR_type == 'average_orientation' or SR_type == 'average_displacement':
            ...
            running_displacement /= self.num_items*(self.num_items - 1)
            running_orientation /= self.num_items*(self.num_items - 1)
